...
dmd.On_s = 33; % each chirp trial is 32 seconds long
dmd.Off_s = 1;
dmd.Power_V = 0.35;
dmd.Iterations = 1;
dmd.Led = 4;
...
pause(5)
%%
ifi = Screen('GetFlipInterval', win);  % Inter-frame interval (e.g., ~0.0167 s for 60 Hz)
...
for trial = 1:p.nTrials
    ...
    vbl=Screen('Flip', win);
    waitFrames = round(p.tSteadyOFF2_s/ ifi);
    ...
    Screen('Flip', win);
    vbl=Screen('Flip', win);
    waitFrames = round(p.tSteadyON_s/ ifi);
    whentime=vbl+(waitFrames)*ifi;
    WaitSecs('UntilTime',whentime);
    ...
    vbl=Screen('Flip', win);
    waitFrames = round(p.tSteadyOFF_s/ ifi);
    whentime=vbl+(waitFrames)*ifi;
     WaitSecs('UntilTime',whentime);
    ...
    vbl=Screen('Flip', win);
    waitFrames = round(p.tSteadyMID_s/ ifi);
    whentime=vbl+(waitFrames)*ifi;
     WaitSecs('UntilTime',whentime);
    ...
    for iT = 0:nPntChirp-1
        ...
        Screen('Flip', win);
        vbl=Screen('Flip', win);
    waitFrames = round(p.durFr_s/ ifi);
    whentime=vbl+(waitFrames)*ifi;
        %WaitSecs(p.durFr_s);
         WaitSecs('UntilTime',whentime);
    end
